...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON team_id = winner_id WHERE year = '2014' UNION SELECT name FROM teams INNER JOIN games ON team_id = opponent_id WHERE year = '2014' ORDER BY name")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON team_id = winner_id WHERE name LIKE 'Co%'")"
